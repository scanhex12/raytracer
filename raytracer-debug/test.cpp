...
TEST_CASE("Classic box", "[raytracer]") {
    ...
    CheckImage(obj_filename, "classic_box/depth1.png", camera_opts, render_opts, std::string(kTestsDir) + std::string("/outp1.png"));
    render_opts.mode = RenderMode::kNormal;
    CheckImage(obj_filename, "classic_box/normal1.png", camera_opts, render_opts, std::string(kTestsDir) + std::string("/outp2.png"));
    camera_opts.look_from = {-0.9, 1.9, -1};
    camera_opts.look_to = {0.0, 0.0, 0};
    render_opts.mode = RenderMode::kDepth;
    CheckImage(obj_filename, "classic_box/depth2.png", camera_opts, render_opts, std::string(kTestsDir) + std::string("/outp3.png"));
    render_opts.mode = RenderMode::kNormal;
    CheckImage(obj_filename, "classic_box/normal2.png", camera_opts, render_opts, std::string(kTestsDir) + std::string("/out4.png"));
}

TEST_CASE("Box with spheres", "[raytracer]") {
    CameraOptions camera_opts(640, 480, 3.1415926 / 3);
